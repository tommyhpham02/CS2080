    ...
    if (state.input1.enabled) {
        if ((ev->type == SAPP_EVENTTYPE_KEY_DOWN) || (ev->type == SAPP_EVENTTYPE_KEY_UP)) {
            ...
            switch (ev->key_code) {
                ...
                case SAPP_KEYCODE_L:
                    state.input1.l = state.input1.anykey = btn_down;
                    break;
...
static void intro_tick(void) {
    ...
    if (after_once(state.intro.started, delay)) {
        ...
        //add command to press l to learn the rules 
    ...
    //rules of the game 
    if (state.input1.l || state.input2.l){
        vid_color_text(i2(5, 5), 3, "RULES AND ADDED FEATURES OF GAME");
        for (int i = 6; i <= 30; i++) {
            vid_color_text(i2(1, i), 4, "                                           ");
        }
        vid_color_text(i2(7, 7), 3, "This is a test");
    }
}
